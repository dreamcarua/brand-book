...
- **Legal lexicon comes first.** In any DreamCar text — here, in email templates, in examples — never write «розіграш», «лотерея», «квиток», «білет», «шанс», «виграй», «джекпот», «казино», «азарт», «ставка», «фарт». Write «учасники», «токени ШІ-сервісу», «автомобіль вручається серед учасників», «власники» (not «переможці»). The canon lives in `sections/legal.html` (section 09, formerly 11B; `legal-safe-lexicon.html` is only a redirect stub), is enforced mechanically by `scripts/brand_lint.py` and in the browser tools by `assets/lexicon.js`.
- Rule hierarchy when two sections disagree: **LAW (09) → VOICE (10) → VISUAL**. The legal lexicon overrides nothing else — nothing overrides it.
...
- Never: hand-edit `print.html`, `assets/search-index.json` or the TOC block in `index.html`; section titles, plates, breadcrumbs and prev/next are also rewritten by `build_nav.py` — they are generated and your edit is lost on the next build.
...
3. `memory/traps.md` — before the first edit of a section, an asset or a workflow. Always. A trap that then changes what you do earns `Memory-Used: traps.md#<slug>` in that commit's message — see Rotation.
...
A trigger, not a phase: it fires mid-task, whenever a system behaves differently from how you were confident it would behave. Write it to `memory/traps.md` **now**, before you finish the thought. This is the one place where writing beats finishing: the symptom is still there at Exit, the reason is not, and the reason is the half worth having.
...
Same shape as any trap — symptom, cause, what to do, date — and the cause is the point: **why** it happened, not only what you saw. About a tool as well as the code, it goes in `memory/tooling.md` too.
...
- `traps.md` ≤ 25 KB (this repo is a code repo: its traps are about one codebase; a project hub accumulates traps across a whole business and gets 50 KB) → one-off incidents older than 90 days move to `memory/traps/archive-<YYYY>-Q<n>.md`. A trap that names a permanent property of the system is evergreen and stays; when in doubt, keep it.
- `tasks.md` ≤ 10 KB (this repo is a code repo: its tasks close when the code is written; a project hub gets 25 KB) → items whose author confirmed them done move to `memory/tasks/done-<YYYY>-MM.md`, verbatim, with the closing date.
- `handoff.md` ≤ 2 KB → emptied on Exit; what survives becomes a line in `tasks.md`.

An item waiting on a person, a partner or money is not a task: it belongs in `memory/open-questions.md`. `tasks.md` is for what someone can act on now.

The budget is a signal, not a licence to break the eligibility rule above. Over budget with nothing eligible: archive nothing, write one line in `memory/open-questions.md` — file, size, budget, nothing eligible under the 90-day rule, needs a human decision — and carry on with the task.
...
| `sections/legal.html` | 09 legal-safe lexicon (formerly 11B) — the canon the linter enforces | before writing any public text |
